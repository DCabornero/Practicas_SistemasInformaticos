CREATE OR REPLACE FUNCTION getTopMonths(productos_min INTEGER, importe_min INTEGER)
  ...
  BEGIN
    RETURN QUERY
    SELECT year, month, amount, prods FROM(
      SELECT date_part('year', orders.orderdate)::INTEGER AS year,
    ...
    WHERE calc2.amount > importe_min OR calc.prods > productos_min;
